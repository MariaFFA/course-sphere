...
const CourseDetailPage = () => {
  ...
  const fetchLessons = async () => {
    try {
      const lessonData = await apiGetLessonsByCourseId(id);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const lessonData = await apiGetLessonsByCourseId(courseData.id);
        setLessons(lessonData);
        ...
        <Grid container spacing={4}>
            ...
            <Paper elevation={2} sx={{ p: 2 }}>
                ...
                {lessons.length > 0 ? lessons.map((lesson, index) => {
                    ...
                    <Typography sx={{ p: 2 }}>Nenhuma aula cadastrada ainda.</Typography>
                )}
                </List>
            </Paper>
            </Grid>
